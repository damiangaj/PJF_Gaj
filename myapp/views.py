...
def addUserToQuizView(request):
    if request.method == 'POST':
        try:
            ...
            print(username)
            Quiz.objects.filter(pk=quiz_).update(user2=username)

            # Tutaj dodaj kod obsługujący dodanie użytkownika do quizu (np. zapis do bazy danych)

            # Zwróć odpowiedź JSON (możesz dostosować do swoich potrzeb)
...
def save_quiz(request):
    if request.method == 'POST':
        try:
            ...
            quiz = Quiz.objects.create(name=quiz_title, user=request.user)
